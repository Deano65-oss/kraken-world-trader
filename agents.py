...
def review_with_gpt4o(pair, price):
    prompt = f"Review {pair} at {price}. Should we buy or sell? Just say 'buy', 'sell', or 'hold'."
...
def review_with_gpt5(pair, price):
    prompt = f"Advanced review for {pair} at {price}. Consider market trends. Buy, sell, or hold?"
...
def add_agent(suggestion):
    ...
    if "new agent" in suggestion.lower():
        review_with_gpt5 = lambda pair, price: client.chat.completions.create(
            model="gpt-4o", messages=[{"role": "user", "content": f"New agent review for {pair} at {price}. Buy, sell, or hold?"}], max_tokens=10
        ).choices[0].message.content.strip().lower() == 'buy'
        logging.info("New agent added.")
